...
def log_mult_fn(freq1, freq2):
  return math.log(freq1, 2) * math.log(freq2, 2)
...
def divide_by_magnitudes_fn(d, words1, words2):
  return float(d) / (len(words1) * len(words2))
...
metrics = {
    ...
    'Sublinear TF': Metric(log_mult_fn, unit_fn, divide_by_magnitudes_fn, default_weight_fn),
}
